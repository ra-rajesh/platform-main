output "public_association_ids" {
  ...
  value       = aws_route_table_association.platform_main_public_rta[*].id
}
...
output "private_association_ids" {
  ...
  value       = aws_route_table_association.platform_main_private_rta[*].id
}

output "public_association_by_subnet" {
  description = "Map: public subnet ID => association ID"
  value       = { for i, a in aws_route_table_association.platform_main_public_rta : var.public_subnet_ids[i] => a.id }
}
